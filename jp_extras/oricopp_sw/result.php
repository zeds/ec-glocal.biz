...
// $Id: result.php by tommy from cs-cart.jp 2015
...
if( preg_match('/^210\.239\.44\.160/', $_SERVER['REMOTE_ADDR']) ) {
    ...
    // 通知件数パラメータが存在する場合
        ...
        for($cnt = 0; $cnt < $number_of_notify; $cnt++){
            ...
            // 注文ステータスがすでに「P（処理中）」に変更されている場合、メールによる通知は実施しない
            ...
            $type = fn_oppsw_get_result_type($num, $_REQUEST);
            ...
            foreach($order_ids_to_process as $order_id){
                ...
                fn_oppsw_set_result_info($info, $num, $type, $_REQUEST);
