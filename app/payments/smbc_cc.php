...
// $Id: smbc_cc.php by tommy from cs-cart.jp 2016
...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
	...
	if( !empty($_REQUEST['PaRes']) && $_REQUEST['process_type'] == '3dsecure_return' ){
		...
		if( empty($processor_data) ){
			// 支払方法に関するデータを取得
			$payment_id = db_get_field("SELECT ?:payments.payment_id FROM ?:payments LEFT JOIN ?:payment_processors ON ?:payment_processors.processor_id = ?:payments.processor_id WHERE ?:payment_processors.processor_script = ?s AND ?:payments.status = 'A'", 'smbc_cc.php');
	...
	if (!PEAR::isError($response)) {
		...
		if( strcmp($smbc_results['rescd'], '000100') === 0){
			...
			$term_url = fn_url("payment_notification.process&payment=smbc_cc&order_id=$order_id&process_type=3dsecure_return&smbcsess=$smbc_sess", AREA, 'current');
...
exit;
		...
		// オーソリが正常に完了した場合
			...
			if (fn_check_payment_script('smbc_cc.php', $order_id)) {
				// DBに保管する支払い情報を生成
