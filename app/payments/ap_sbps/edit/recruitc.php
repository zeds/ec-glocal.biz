...
if ($mode == 'place_order' && AREA == 'A' && Registry::get('runtime.action') != 'save') {
    ...
    // 実行チェック
    if ((round($order_data['total']) < (int)$order_info['total']) || !$sbps->valid_mode_exec($order_id, 'multiple_refund', 'credit')) {
        // 注文情報ロールバック
    ...
    $refund_rowno = fn_ap_sbps_get_refund_rowno($order_id, 'credit');
    ...
    $sbps->cancel_request($tracking_id, 'recruitc', $add_data, 'multiple_refund');
