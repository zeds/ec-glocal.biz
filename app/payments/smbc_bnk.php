...
// $Id: smbc_bnk.php by tommy from cs-cart.jp 2016
// SMBCファイナンスサービス（銀行振込）
...
if( ($mode == 'place_order' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
	...
	$params = array();
	$params = fn_smbcks_get_params('bnk', $order_id, $order_info, $processor_data);
	...
	if (!PEAR::isError($response)) {
		...
		if( strcmp($smbc_results['rescd'], '000000') !== 0 ){
			...
			// 注文処理ページへリダイレクト
			fn_set_notification('E', __('jp_smbc_bnk_error'), __('jp_smbc_bnk_failed') . '<br />' . $smbc_results['rescd'] . ' : ' . $smbc_results['res']);
			$return_url = fn_lcjp_get_error_return_url();
			fn_redirect($return_url, true);

		// 銀行振込が正常終了した場合
		}else{
			if (fn_check_payment_script('smbc_bnk.php', $order_id)) {
				// DBに保管する支払い情報を生成
				fn_smbcks_format_payment_info('bnk', $order_id, $order_info['payment_info'], $smbc_results, true);
	...
	// 銀行振込データ送信が異常終了した場合
		...
		// 注文処理ページへリダイレクト
		fn_set_notification('E', __('jp_smbc_bnk_error'), __('jp_smbc_bnk_failed'));
		$return_url = fn_lcjp_get_error_return_url();
		fn_redirect($return_url, true);
	}
}
