...
// $Id: smbc_ccreg.php by tommy from cs-cart.jp 2016
// SMBCファイナンスサービス（登録済みカード決済）
...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
	...
	if( !empty($_REQUEST['PaRes']) && $_REQUEST['process_type'] == '3dsecure_return' ){
		...
		if( empty($processor_data) ){
			// 支払方法に関するデータを取得
			$payment_id = db_get_field("SELECT ?:payments.payment_id FROM ?:payments LEFT JOIN ?:payment_processors ON ?:payment_processors.processor_id = ?:payments.processor_id WHERE ?:payment_processors.processor_script = ?s AND ?:payments.status = 'A'", 'smbc_ccreg.php');
    ...
    // その他の場合
        ...
        // 処理タイプが登録済みカード決済の場合
            ...
            // SMBCファイナンスサービスに送信するパラメータをセット
            $params = array();
            $params = fn_smbcks_get_params('ccreg', $order_id, $order_info, $processor_data);
            $action = 'checkout';
        }
    }
	...
	if (!PEAR::isError($response)) {
		...
		if( strcmp($smbc_results['rescd'], '000100') === 0){
			...
			$term_url = fn_url("payment_notification.process&payment=smbc_ccreg&order_id=$order_id&process_type=3dsecure_return&smbcsess=$smbc_sess", AREA, 'current');
            ///////////////////////////////////////////////
...
exit;
		...
		if( strcmp($smbc_results['rescd'], '000000') !== 0){
			...
			// 注文処理ページへリダイレクト
			fn_set_notification('E', __('jp_smbc_cc_error'), __('jp_smbc_ccreg_failed') . '<br />' . $smbc_results['rescd'] . ' : ' . $smbc_results['res']);
			$return_url = fn_lcjp_get_error_return_url();
			fn_redirect($return_url, true);

		// オーソリが正常に完了した場合
		}else{
			if (fn_check_payment_script('smbc_ccreg.php', $order_id)) {
				// DBに保管する支払い情報を生成
