...
// $Id: smbc_ps.php by tommy from cs-cart.jp 2016
// SMBCファイナンスサービス（払込票決済）
...
if( ($mode == 'place_order' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
	...
	$params = array();
	$params = fn_smbcks_get_params('ps', $order_id, $order_info, $processor_data);
	...
	if (!PEAR::isError($response)) {
		...
		if( strcmp($smbc_results['rescd'], '000000') !== 0 ){
			...
			// 注文処理ページへリダイレクト
			fn_set_notification('E', __('jp_smbc_ps_error'), __('jp_smbc_ps_failed') . '<br />' . $smbc_results['rescd'] . ' : ' . $smbc_results['res']);
			$return_url = fn_lcjp_get_error_return_url();
			fn_redirect($return_url, true);

		// 払込票決済が正常終了した場合
		}else{
			if (fn_check_payment_script('smbc_ps.php', $order_id)) {
                // DBに保管する支払い情報を生成
                fn_smbcks_format_payment_info('ps', $order_id, $order_info['payment_info'], $smbc_results, true);
	...
	// 払込票決済データ送信が異常終了した場合
		...
		// 注文処理ページへリダイレクト
		fn_set_notification('E', __('jp_smbc_ps_error'), __('jp_smbc_ps_failed'));
		$return_url = fn_lcjp_get_error_return_url();
		fn_redirect($return_url, true);
	}
}
