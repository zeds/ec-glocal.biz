...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if ($mode == 'process') {
        ...
        if (($_REQUEST['merchant_id'] == $processor_data['processor_params']['merchant_id']) && ($_REQUEST['amount'] == round($order_info['total'])) && $_REQUEST['res_result'] == 'OK' && empty($_REQUEST['res_err_code'])) {
            // 正常終了した場合
            if (fn_check_payment_script('ap_sbps_rb_link.php', $order_id)) {
    ...
    $attr = [
        ...
        'auto_charge_type' => '1',
        'div_settele' => '0',
        'camp_type' => $processor_data['processor_params']['camp_type'],
        'success_url' => html_entity_decode(fn_url("payment_notification.process&payment=ap_sbps_rb_link&order_id={$order_id}", AREA, 'current'), ENT_QUOTES, 'UTF-8'),
        'cancel_url' => html_entity_decode(fn_url("payment_notification.cancelled&payment=ap_sbps_rb_link&order_id={$order_id}", AREA, 'current'), ENT_QUOTES, 'UTF-8'),
        'error_url' => html_entity_decode(fn_url("payment_notification.error&payment=ap_sbps_rb_link&order_id={$order_id}", AREA, 'current'), ENT_QUOTES, 'UTF-8'),
        'pagecon_url' => html_entity_decode(fn_lcjp_get_return_url('/jp_extras/ap_sbps/rb_result.php'), ENT_QUOTES, 'UTF-8'),
    ];
