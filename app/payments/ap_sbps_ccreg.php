...
if ($mode == 'place_order' && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save'))) {
    // 支払い方法チェック
    if (!fn_check_payment_script('ap_sbps_ccreg.php', $order_id)) {
        if (AREA === 'C') {
    ...
    if (AREA === 'C'){
        ...
        $user_id = !empty(Tygh::$app['session']['cart']['user_data']['user_id']) ? Tygh::$app['session']['cart']['user_data']['user_id'] : null;
    }

    $payment_method_data = fn_ap_sbps_get_quickpay_payment_method_data($user_id);
    ...
    if (AREA === 'C') {
        ...
        foreach ($payment_order_ids as $payment_order_id) {
            ...
            if (!empty($credit_payment_info) && !in_array($credit_payment_info['payment_status'], [SBPS_PAYMENT_STATUS_AUTH_CANCEL, SBPS_PAYMENT_STATUS_REFUNDED], true)) {
                continue;
            }
            ...
            // 初期化
            $sbps = new SbpsCredit($payment_order_id, $payment_method_data['processor_params']);
        ...
        // 初期化
        $sbps = new SbpsCredit($order_id, $payment_method_data['processor_params']);
