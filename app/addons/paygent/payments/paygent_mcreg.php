...
// $Id: paygent_mcreg.php by tommy from cs-cart.jp 2016
// ペイジェント（登録済みカード決済（多通貨））
...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
	...
	if( !empty($_REQUEST['order_id']) && isset($_REQUEST['result']) && $_REQUEST['process_type'] == '3dsecure_return' ){
        ...
        // カード会社側画面の3Dセキュア処理が完了した場合
            ...
            if( fn_check_payment_script('paygent_mcreg.php', $order_id) ){
                // 決済IDを注文データに登録
                if( !empty($_REQUEST['payment_id']) ){
                    $res_array = array();
                    $res_array['payment_id'] = $_REQUEST['payment_id'];
                    // DBに保管する支払い情報を生成
                    fn_pygnt_format_payment_info('mcreg', $order_id, array(), $res_array);
                }
    ...
    // その他の場合
        ...
        if( Registry::get('runtime.mode') == 'place_order' && Registry::get('runtime.controller') == 'order_management') {
            ...
            // 金額変更の場合
            if ($is_cc_changeable) {
                fn_pygnt_mc_change_amount($order_id);
                return true;
            }
        }
        ...
        // 3Dセキュア認証を実施する場合
            ...
            if( $p->hasResNext() ) {
                ...
                if( !empty($res_array['out_acs_html']) ){
                    ...
                    // DBに保管する支払い情報を生成
                    fn_pygnt_format_payment_info('mcreg', $order_id, $payment_info, $res_array);
