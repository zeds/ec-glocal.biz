...
// $Id: paygent_ccregtkn.php by tommy from cs-cart.jp 2016
// ペイジェント（登録済みカード決済）
    ...
    if (empty($order_info['payment_info']['errorCode'])) {
        ...
        if( !empty($_REQUEST['order_id']) && isset($_REQUEST['result']) && $_REQUEST['process_type'] == '3dsecure_return' ){
            ...
            // カード会社側画面の3Dセキュア処理が完了した場合
                ...
                if( fn_check_payment_script('paygent_ccregtkn.php', $order_id) || fn_check_payment_script('paygent_cctkn.php', $order_id)){
                    ...
                    if( !empty($_REQUEST['payment_id']) ){
                        ...
                        // DBに保管する支払い情報を生成
                        fn_pygnt_format_payment_info('ccreg', $order_id, array(), $res_array);
                    }
        ...
        // データ送信が正常に終了した場合
            ...
            // 3Dセキュア認証を実施する場合
                ...
                if( $p->hasResNext() ) {
                    ...
                    if( !empty($res_array['out_acs_html']) ){
                        ...
                        // DBに保管する支払い情報を生成
                        fn_pygnt_format_payment_info('ccreg', $order_id, $payment_info, $res_array);
            ...
            // ペイジェントによるオーソリが完了した場合
                ...
                // 注文IDと利用した支払方法がマッチした場合
                if( fn_check_payment_script('paygent_ccregtkn.php', $order_id) ){
