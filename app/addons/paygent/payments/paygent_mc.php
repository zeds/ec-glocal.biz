...
// $Id: paygent_mc.php by tommy from cs-cart.jp 2016
// ペイジェント（多通貨クレジットカード決済）
...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
	...
	if( !empty($_REQUEST['order_id']) && isset($_REQUEST['result']) && $_REQUEST['process_type'] == '3dsecure_return' ){
        ...
        // カード会社側画面の3Dセキュア処理が完了した場合
            ...
            if (fn_check_payment_script('paygent_mc.php', $order_id)) {
                // 決済IDを注文データに登録
                if( !empty($_REQUEST['payment_id']) ){
                    $res_array = array();
                    $res_array['payment_id'] = $_REQUEST['payment_id'];
                    // DBに保管する支払い情報を生成
                    fn_pygnt_format_payment_info('mc', $order_id, array(), $res_array);
                }
        ...
        // 3Dセキュア認証を実施する場合
            ...
            if( $p->hasResNext() ) {
                ...
                if( !empty($res_array['out_acs_html']) ){
                    ...
                    // DBに保管する支払い情報を生成
                    fn_pygnt_format_payment_info('mc', $order_id, $payment_info, $res_array);
