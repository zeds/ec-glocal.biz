...
// $Id: payjp_cc_manager.php by takahashi from cs-cart.jp 2020
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if( !empty($_REQUEST['order_ids']) ){
        ...
        // 処理コードに応じて処理を実行
        switch($type){
            case 'cc_sales_confirm':    // 一括支払確定
            case 'cc_auth_cancel':      // 一括認証取消
            case 'cc_sales_cancel':     // 一括支払取消
                $is_valid_orders_exist = false;
                foreach ($_REQUEST['order_ids'] as $k => $v) {
                    if( fn_payjp_send_cc_request($v, $type) ){
                        // 処理実行フラグをtrueにセット
                ...
                if( !$is_valid_orders_exist ){
                    ...
                    fn_set_notification('E', __('jp_payjp_' . $type . '_error'), __('jp_payjp_cc_data_not_exists'));
                }
    ...
    return array(CONTROLLER_STATUS_OK, "payjp_cc_manager.manage");
}
...
switch($mode){
    ...
    case 'cc_sales_confirm':
        fn_payjp_send_cc_request($_REQUEST['order_id']);
        return array(CONTROLLER_STATUS_REDIRECT);
        break;
    // 与信取消
    case 'cc_auth_cancel':
        fn_payjp_send_cc_request($_REQUEST['order_id'], 'cc_auth_cancel');
        return array(CONTROLLER_STATUS_REDIRECT);
        break;
    // 売上取消
    case 'cc_sales_cancel':
        fn_payjp_send_cc_request($_REQUEST['order_id'], 'cc_sales_cancel');
        return array(CONTROLLER_STATUS_REDIRECT);
        break;
    // 一覧ページ
    case 'manage':
        ...
        $payjp_total = db_get_field("SELECT COUNT(*) FROM ?:jp_payjp_cc_status");

        // GMO PG（クレジットカード決済）で決済した注文が存在する場合
        if( !empty($payjp_total) ){
            // クレジットカード決済を用いた注文を抽出
