...
// $Id: gmomp_cc_manager.php by tommy from cs-cart.jp 2015
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        switch($type){
            ...
            case 'cc_sales_cancel':     // 一括売上取消
                ...
                foreach ($_REQUEST['order_ids'] as $k => $v) {
                    if( fn_gmomp_send_cc_request($v, $type) ){
                        // 処理実行フラグをtrueにセット
                ...
                if( !$is_valid_orders_exist ){
                    ...
                    fn_set_notification('E', __('jp_gmo_multipayment_' . $type . '_error'), __('jp_gmo_multipayment_cc_data_not_exists'));
                }
    ...
    return array(CONTROLLER_STATUS_OK, "gmomp_cc_manager.manage");
}
...
switch($mode){
    ...
    case 'cc_sales_confirm':
        fn_gmomp_send_cc_request($_REQUEST['order_id']);
        return array(CONTROLLER_STATUS_REDIRECT);
        break;
    // 与信取消
    case 'cc_auth_cancel':
        fn_gmomp_send_cc_request($_REQUEST['order_id'], 'cc_auth_cancel');
        return array(CONTROLLER_STATUS_REDIRECT);
        break;
    // 売上取消
    case 'cc_sales_cancel':
        fn_gmomp_send_cc_request($_REQUEST['order_id'], 'cc_sales_cancel');
        return array(CONTROLLER_STATUS_REDIRECT);
        break;
    // 一覧ページ
    case 'manage':
        ...
        $gmomp_total = db_get_field("SELECT COUNT(*) FROM ?:jp_gmomp_cc_status");

        // GMO PG（クレジットカード決済）で決済した注文が存在する場合
        if( !empty($gmomp_total) ){
            // クレジットカード決済を用いた注文を抽出
