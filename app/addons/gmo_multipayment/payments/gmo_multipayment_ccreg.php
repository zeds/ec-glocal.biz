...
// $Id: gmo_multipayment_ccreg.php by tommy from cs-cart.jp 2015
// GMOマルチペイメントサービス（登録済みクレジットカード決済）
...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
	...
	if( !empty($_REQUEST['PaRes']) && $_REQUEST['process_type'] == '3dsecure_return' ){
		...
		if( empty($processor_data) ){
			// 支払方法に関するデータを取得
			$payment_id = db_get_field("SELECT ?:payments.payment_id FROM ?:payments LEFT JOIN ?:payment_processors ON ?:payment_processors.processor_id = ?:payments.processor_id WHERE ?:payment_processors.processor_script = ?s AND ?:payments.status = 'A'", 'gmo_multipayment_ccreg.php');
        ...
        if (!empty($exec_result)) {
            ...
            if (!empty($gmomp_exec_results['OrderID']) && !empty($_REQUEST['order_id']) && empty($gmomp_exec_results['ErrCode']) && empty($gmomp_exec_results['ErrInfo'])) {
                ...
                if (fn_check_payment_script('gmo_multipayment_ccreg.php', $order_id)) {
                    // 注文確定処理
                    $pp_response = array();
	...
	if (!empty($entry_result)) {
        ...
        if( !empty($gmomp_entry_results['AccessID']) && !empty($gmomp_entry_results['AccessPass']) && empty($gmomp_entry_results['ErrCode']) && empty($gmomp_entry_results['ErrInfo']) ){
            ...
            $params = array();
            $params = fn_gmomp_get_params('exectran_ccreg', $order_id, $order_info, $processor_data);
            ...
            if (!empty($exec_result)) {
                ...
                if( $gmomp_exec_results['ACS'] == 1 && $gmomp_exec_results['ACSUrl'] && $gmomp_exec_results['PaReq'] && $gmomp_exec_results['MD'] ){
                    ...
                    $term_url = fn_url("payment_notification.process&payment=gmo_multipayment_ccreg&order_id=$order_id&process_type=3dsecure_return", AREA, 'current');
