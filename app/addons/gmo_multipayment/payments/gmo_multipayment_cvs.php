...
// $Id: gmo_multipayment_cvs.php by tommy from cs-cart.jp 2015
// GMOマルチペイメントサービス（コンビニ決済）
...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
    ...
    $params = array();
    $params = fn_gmomp_get_params('entrytrancvs', $order_id, $order_info, $processor_data);
    $gmomp_order_id = $params['OrderID'];

	// 取引登録
    $entry_result = fn_gmomp_send_request('entrytrancvs', $params, $processor_data['processor_params']['mode']);
	...
	if (!empty($entry_result)) {
        ...
        if( !empty($gmomp_entry_results['AccessID']) && !empty($gmomp_entry_results['AccessPass']) && empty($gmomp_entry_results['ErrCode']) && empty($gmomp_entry_results['ErrInfo']) ){
            ...
            $params = array();
            $params = fn_gmomp_get_params('exectrancvs', $order_id, $order_info, $processor_data);
            ...
            // 決済実行
            $exec_result = fn_gmomp_send_request('exectrancvs', $params, $processor_data['processor_params']['mode']);
            ...
            if (!empty($exec_result)) {
                ...
                if( !empty($gmomp_exec_results['OrderID']) && empty($gmomp_exec_results['ErrCode']) && empty($gmomp_exec_results['ErrInfo']) ){
                    ...
                    if (fn_check_payment_script('gmo_multipayment_cvs.php', $order_id)) {
                        // 注文確定処理
                        $pp_response = array();
                        ...
                        // DBに保管する支払い情報を生成
                        fn_gmomp_format_payment_info('cvs', $order_id, $order_info['payment_info'], $gmomp_exec_results);

                        // コンビニ決済に関するメッセージを表示
                        fn_set_notification('I', __('jp_gmo_multipayment_cvs_notification_title'),  __('jp_gmo_multipayment_cvs_notification_message', array('[email]' => $params['MailAddress'])));
	...
	// リクエスト送信が異常終了した場合
		...
		// 注文処理ページへリダイレクト
		fn_set_notification('E', __('jp_gmo_multipayment_cvs_error'), __('jp_gmo_multipayment_cvs_failed'));
		$return_url = fn_lcjp_get_error_return_url();
		fn_redirect($return_url, true);
	}
}
