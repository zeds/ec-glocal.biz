...
// デフォルトのSEO名（page-xxx）だとブログの詳細が開けないバグを修正
...
{
    // Modified by takahashi from cs-cart.jp 2021
    //SQLインジェクション不正アクセス防止 BEGIN
    if($req['dispatch'] == 'categories.view') {
        if (!empty($req['category_id']) && !is_numeric($req['category_id'])) {
            echo 'INVALID ACCESS';
            exit();
        }
    }

    if($req['dispatch'] == 'products.view') {
        if (!empty($req['product_id']) && !is_numeric($req['product_id'])) {
            echo 'INVALID ACCESS';
            exit();
        }
    }

    if(!empty($req['sl']) && strlen($req['sl']) > 3){
        echo 'INVALID ACCESS';
        exit();
    }
    //SQLインジェクション不正アクセス防止 END
...
function fn_seo_get_parent_items_path($seo_var, $object_type, $object_id, $company_id = null, $lang_code = CART_LANGUAGE)
{
    ...
    if (!empty($id_path)) {
        ...
        foreach ($path_ids as $v) {
            ...
            ///////////////////////////////////////////////////////////////
            fn_set_hook('jp_object_type_for_name_post', $object_type, $object_type_for_name);
            ///////////////////////////////////////////////////////////////
