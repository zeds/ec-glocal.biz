...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_REQUEST['order_ids'])) {
        ...
        foreach ($_REQUEST['order_ids'] as $order_id) {
            $sbps = new SbpsPrepaid();

            if ($sbps->valid_mode_exec($order_id, $mode, 'prepaid'))  {
                if (!$valid_process) {
                    $valid_process = true;
                }
                ...
                $payment_info = fn_ap_sbps_get_payment_info($order_id, 'prepaid');
                ...
                if (!empty($sbps->errors)) {
                    ...
                    fn_ap_sbps_set_sbps_payment_info($order_id, ['payment_status' => SBPS_PAYMENT_STATUS_REFUNDED], 'prepaid');
                    fn_ap_sbps_set_order_data($order_id, fn_ap_sbps_get_payment_info($order_id, 'prepaid'), 'prepaid');
                }
            }
        }
