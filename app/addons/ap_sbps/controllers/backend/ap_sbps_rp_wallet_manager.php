...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_REQUEST['order_ids'])) {
        ...
        foreach ($_REQUEST['order_ids'] as $order_id) {
            $sbps = new SbpsRpWallet();

            if ($mode !== 'cancel_contract' && $sbps->valid_rp_mode_exec($order_id, $mode, 'rp_wallet'))  {
                if (!$valid_process) {
                    $valid_process = true;
                }
                ...
                $payment_info = fn_ap_sbps_get_payment_info($order_id, 'rp_wallet');
                ...
                if (!empty($sbps->errors)) {
                    ...
                    fn_ap_sbps_set_sbps_payment_info($order_id, ['payment_status' => $payment_status], 'rp_wallet');
                    fn_ap_sbps_set_order_data($order_id, fn_ap_sbps_get_payment_info($order_id, 'rp_wallet'), 'rp_wallet');
                }
            }
        }
...
if ($mode === 'manage' || empty($_REQUEST['order_id'])) {
    ...
    if ($sbps->valid_rp_mode_exec($order_id, $mode, 'rp_wallet'))  {
        // 処理情報取得
        $processor_data = fn_ap_sbps_get_processor_data($order_id);
