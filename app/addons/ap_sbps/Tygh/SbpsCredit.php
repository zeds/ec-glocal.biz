...
class SbpsCredit extends Sbps
{
    ...
    public function cancel_request($tracking_id, $pay_method, $add_data = [], $mode = 'cancel')
    {
        ...
        if (!in_array($pay_method, ['unionpay', 'recruitc'], true)) {
            $data['processing_datetime'] = date('YmdHis');
        }
     ...
     * @return array
     */
    public function cc_reference_request($user_id)
    {
        $data = [
            'merchant_id' => $this->processor['merchant_id'],
            'service_id' => $this->processor['service_id'],
            'cust_code' => $this->format_cust_code($user_id),
        ...
        return $this->send_request('MG02-00104-101', $data);
    }
     ...
     * @return array
     */
    public function cc_delete_request($user_id)
    {
        $data = [
            'merchant_id' => $this->processor['merchant_id'],
            'service_id' => $this->processor['service_id'],
