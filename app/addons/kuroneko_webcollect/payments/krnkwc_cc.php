...
// $Id: krnkwc_cc.php by tommy from cs-cart.jp 2016
...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
	...
	if( !empty($_REQUEST['3D_TRAN_ID']) && !empty($_REQUEST['COMP_CD']) && $_REQUEST['pt'] == '3ds' ){
        ...
        ///////////////////////////////////////////////////////////////////////////
        $tds_params = array();
        $tds_params['order_no'] = Tygh::$app['session']['krnkwc_cc'][$order_id]['order_no'];
        ...
        $tds_params['three_d_token'] = Tygh::$app['session']['krnkwc_cc'][$order_id]['three_d_token'];
        ///////////////////////////////////////////////////////////////////////////
        // クレジット決済登録（3Dセキュア後）に使用するパラメータをセット EOF
        ///////////////////////////////////////////////////////////////////////////

        if( !empty(Tygh::$app['session']['krnkwc_cc'][$order_id]['auth_key']) ) $krnkwc_auth_key = Tygh::$app['session']['krnkwc_cc'][$order_id]['auth_key'];

        // 受付番号と3Dトークンを保存するセッション変数をクリア
        unset(Tygh::$app['session']['krnkwc_cc']);
        ...
        $tds_result = fn_krnkwc_send_request('credit3d', $params, $processor_data['processor_params']['mode']);
        ...
        if (!empty($tds_result)) {
            ...
            if ( $tds_result['returnCode'] == 0 ) {
                ...
                if (fn_check_payment_script('krnkwc_cc.php', $order_id)) {
                    // 注文確定処理
                    $pp_response = array();
	...
	// クレジット決済登録（通常）
    $cc_result = fn_krnkwc_send_request('credit', $params, $processor_data['processor_params']['mode']);
    ...
    if (!empty($cc_result)) {
        ...
        if ( $return_code == 0 ) {
            ...
            if( !empty($cc_result['threeDAuthHtml']) ){
                ...
                if( !empty($cc_result['threeDToken']) ) Tygh::$app['session']['krnkwc_cc'][$order_id]['three_d_token'] = $cc_result['threeDToken'];
                if( !empty($params['order_no']) ) Tygh::$app['session']['krnkwc_cc'][$order_id]['order_no'] = $params['order_no'];
                if( !empty($params['authentication_key']) ) Tygh::$app['session']['krnkwc_cc'][$order_id]['auth_key'] = $params['authentication_key'];
