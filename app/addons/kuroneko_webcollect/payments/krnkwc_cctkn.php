...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
    ...
    if(empty($order_info['payment_info']['errorCd'])) {
        ...
        if (!empty($_REQUEST['3D_TRAN_ID']) && !empty($_REQUEST['COMP_CD']) && $_REQUEST['pt'] == '3ds') {
            ...
            $krnkwc_ccrtn = db_get_array("select three_d_token, order_no, auth_key from ?:jp_krnkwc_3dsecure WHERE order_id = ?i", $order_id);
            ...
            if (!empty($krnkwc_ccrtn[0]['auth_key'])) $krnkwc_auth_key = $krnkwc_ccrtn[0]['auth_key'];
        ...
        if (!empty($cc_result)) {
            ...
            if ($return_code == 0) {
                ...
                if (!empty($cc_result['threeDAuthHtml'])) {
                    ...
                    ///////////////////////////////////////////////
                    // テーブルに3Dトークンと受付番号を保持
                    db_query("REPLACE INTO ?:jp_krnkwc_3dsecure(order_id, three_d_token, order_no, auth_key) VALUES(?i, ?s, ?s, ?s)", $order_id, $cc_result['threeDToken'], $params['order_no'], $params['authentication_key']);
                    ///////////////////////////////////////////////
                    // Modified by takahashi from cs-cart.jp 2022 EOF
                    ///////////////////////////////////////////////
