...
// $Id: omise_ccreg.php by takahashi from cs-cart.jp 2017
// Omise（登録済みクレジットカード決済）
...
if( $mode == 'place_order' && AREA == 'C' ){
    ...
    if(!empty($member_id)){
        ...
        if (!empty($exec_result)) {
            ...
            if ($exec_result['status'] == 'successful' || $exec_result['status'] == 'pending') {
                ...
                if (fn_check_payment_script('omise_ccreg.php', $order_id)) {
                    // 注文確定処理
                    $pp_response = array();
    ...
    // 会員IDが存在しない場合
        ...
        // 注文処理ページへリダイレクト
        fn_set_notification('E', __('error'), __('jp_omise_cc_invalid'));
        $return_url = fn_lcjp_get_error_return_url();
        fn_redirect($return_url, true);
    }
