...
// $Id: sonypayment_carrier_rb.php by takahashi from cs-cart.jp 2018
// ソニーペイメントキャリア（継続課金）
use Tygh\Registry;
...
if( ($mode == 'place_order' || $mode == 'process') && (AREA == 'C') ){
    ...
    if( !empty($_REQUEST['EncryptValue']) && !empty($_REQUEST['process_type']) ){
        ...
        if (!empty($result_params['TransactionId'])) {
            ...
            if( $process_type == 'order_complete' ) {
                ...
                if ($result_params['ResponseCd'] == 'OK') {
                    ...
                    if (fn_check_payment_script('sonypayment_carrier_rb.php', $order_id)) {
                        // 後続処理のための ProcessId と ProcessPass をDBに保存
                        fn_sonyc_update_set_process_info($order_id, $result_params);
                        // DBに保管する支払い情報を生成
                        fn_sonyc_format_payment_info('rb', $order_id, $order_info['payment_info'], $result_params);
            ...
            elseif( $process_type == 'order_cancel' ) {
                ...
                if ($result_params['ResponseCd'] != 'OK') {
                    ...
                    fn_set_notification('E', __('jp_sonypayment_carrier_rb_error'), __('jp_sonypayment_carrier_rb_canceled') . '<br />' . $sln_err_msg . '<br />' . __('jp_sonypayment_carrier_clear_cart'));
            ...
            elseif( $process_type == 'order_error' ) {
                ...
                if ($result_params['ResponseCd'] != 'OK') {
                    ...
                    // 注文処理ページへリダイレクト
                    $sln_err_msg = fn_sonyc_get_err_msg($result_params['ResponseCd']);
                    fn_set_notification('E', __('jp_sonypayment_carrier_rb_error'), __('jp_sonypayment_carrier_rb_failed') . '<br />' . $sln_err_msg . '<br />' . __('jp_sonypayment_carrier_clear_cart'));
                    $return_url = fn_lcjp_get_error_return_url();
                    fn_redirect($return_url, true);
                }
            }
            ...
            // 注文処理ページへリダイレクト
            fn_set_notification('E', __('jp_sonypayment_carrier_rb_error'), __('jp_sonypayment_carrier_rb_invalid'));
            $return_url = fn_lcjp_get_error_return_url();
            fn_redirect($return_url, true);
        }
        ...
        // ソニーペイメントキャリアに送信するパラメータをセット
        $params = fn_sonyc_get_params('rb', $order_info, $processor_data);
        $action = 'rb';
