   ...
   function getUserPages($writableonly = false) {
    ...
    function getUserProfile() {
        try {
            ...
            // 日本語の氏名を取得
            /////////////////////////////////////////////////
            // 日本語の氏名が取得できた場合は姓名を入れ替え
            // 【メモ】CS-Cart日本語版では複数言語利用時の姓名表示順を統一するために"first_name" に姓、"last_name" に名を登録している
            if( $data['locale'] == 'ja_JP' ){
                $_tmp_lastname = $data['first_name'];
                $_tmp_firstname = $data['last_name'];
                $data['last_name'] = $_tmp_lastname;
                $data['first_name'] = $_tmp_firstname;
            }
            /////////////////////////////////////////////////
