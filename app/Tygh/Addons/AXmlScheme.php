        ...
        if (!$errors) {
            ...
            foreach ($lang_queries as $table_name => $queries) {
                ...
                if (isset($queries[$default_lang])) {
                    ...
                    // 【説明】
                    // 関数 "fn_clone_language_values" は、各アドオンで追加・更新される各種
                    // descriptionテーブルのデータを言語種類にかかわらず強制的に定数
                    // DEFAULT_LANGUAGE に設定された言語のデータに書き換える。
                    // そのため、'common_descriptions'、'page_descriptions'、'privilege_descriptions'
                    // に設定した日本語データが英語に書き戻されてしまう。
                    // この不具合を回避するために上記テーブルについては日本語データが書き換え
                    // られないように関数 "fn_clone_language_values" の実行をスキップする
                    //
                    // 【開発上の注意】
                    // "fn_clone_language_values" をスキップすると、日本語データが用意されて
                    // いないレコードについては英語データをベースにしたレコードの複製が実行
                    // されず、レコードが欠落してしまう。
                    // そのため、上記3テーブルについてはすべてのレコードについて日本語データを
                    // 用意しておく必要がある
                    //
                    // 【今後の対応について】
                    // そもそも各descriptionテーブルのデータを強制的に英語に書き換える
                    // 関数 "fn_clone_language_values" の利用は好ましくない。
                    // 実行されるクエリを解析し、当該クエリが追加・更新する英語レコードに関連
                    // する各言語向けレコードのみを英語に書き換える関数を新たに開発する必要
                    // がある
                    /////////////////////////////////////////////////////////////////////////
                        ...
                        foreach ($languages as $lang_code => $lang_data) {
                            switch( $default_query['table'] ){
                                case 'common_descriptions':
                                case 'page_descriptions':
                                case 'privilege_descriptions':
                                    if( $lang_code == 'ja') $skip_clone = true;
                                    break;
                                default:
                                    // do nothing
                            }
...
LanguageHelper::cloneLanguageValues((string) $default_query['table'], $lang_code,
    ...
    /**
     * Returns true if supplier tag is exist.
     *
     * @return bool
     */
    public function hasSupplier()
    {
        return isset($this->_xml->supplier);
    }
    ...
    {
        $core_addons = array_filter(Snapshot::getCoreAddons(), static function ($addon) {
