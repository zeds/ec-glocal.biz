...
function fn_update_product($product_data, $product_id = 0, $lang_code = CART_LANGUAGE)
{
    ...
    if (fn_allowed_for('ULTIMATE') && Registry::get('runtime.company_id') && !empty($product_company_id) && Registry::get('runtime.company_id') != $product_company_id && !empty($_product_id)) {
        ...
        if (empty($product_id)) {
            ...
            $is_status_changed_by_tools = isset($product_data['to_status_by_tools']);
            if ($is_status_changed_by_tools) {
                $new_status = $product_data['to_status_by_tools'];
            }
            $is_need_check_status = $old_status !== $new_status || $is_status_changed_by_tools;

            $is_need_check_subscribes = ($is_need_check_amount || $is_need_check_status)
