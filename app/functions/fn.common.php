...
// Modified to fix bug by tommy from cs-cart.jp 2017
// ペイパルウェブペイメントプラスからの戻りの場合はCSRFチェックの対象外とする

// Modified by takahashi from cs-cart.jp 2022
// 商品のプレビューがhttpになる問題を修正
// 一括アップデートで入荷通知が送信される問題を修正
...
function fn_parse_date($timestamp, $end_time = false)
{
    if (!empty($timestamp)) {
        ...
        if (count($ts) == 3) {
            ...
            ///////////////////////////////////////////////////////////////
            fn_set_hook('jp_post_format_timestamp', $timestamp, $h, $m, $s, $ts);
            ///////////////////////////////////////////////////////////////
...
function fn_get_currencies_list($params = array(), $area = AREA, $lang_code = CART_LANGUAGE)
{
    ...
    if ($params['currency_id']) {
        $condition .= db_quote(' AND currency_id IN (?n)', (array) $params['currency_id']);
    }
...
function fn_get_preview_url($uri, $object_data, $user_id)
{
    ...
    if ($object_data['status'] != 'A' || fn_allowed_for('MULTIVENDOR') || !empty($object_data['usergroup_ids'])) {
        ...
        // 商品のプレビューがhttpになる問題を修正
        ///////////////////////////////////////////////
        $preview_url = fn_url($uri, 'C', 'current', DESCR_SL);
        ///////////////////////////////////////////////
...
function fn_csrf_validate_request($params)
{
    ...
    if ($validation_required) {
        if ($params['server']['REQUEST_METHOD'] == 'POST'
            ...
            /////////////////////////////////////////////////////////////
            // Modified to fix bug by tommy from cs-cart.jp 2017 BOF
            // ペイパルウェブペイメントプラスからの戻りの場合はCSRFチェックの対象外とする
            /////////////////////////////////////////////////////////////
            if( !empty($params['request']['pwpp_tx']) && $params['request']['dispatch'] == 'checkout.place_order' ){
                // do nothing
            }else{
                return false;
            }
            /////////////////////////////////////////////////////////////
            // Modified to fix bug by tommy from cs-cart.jp 2017 EOF
            /////////////////////////////////////////////////////////////
        }
    }
