...
// Modified for Japanese Ver by takahashi from cs-cart.jp 2018
// 複数ショップで同じEメールのお客様が存在する場合に Auth API で見つからないバグを修正

// Modified by takahashi from cs-cart.jp 2021
// 他の出品者商品を出品者データ管理者で購入した際の問題を修正

// Modified by takahashi from cs-cart.jp 2022
// 配送先/請求先のEメールタイプフィールドが表示されないバグを修正

// Modified by takahashi from cs-cart.jp 2022
// 複数住所許可時、追加フィールドの値が同一になる問題を修正
...
function fn_get_user_info($user_id, $get_profile = true, &$profile_id = null)
{
    ...
    if ($get_profile && !empty($profile_data['profile_id'])) {
        ...
        // 複数住所許可時、追加フィールドの値が同一になる問題を修正
        ///////////////////////////////////////////////
        $profile = fn_get_profile_fields_data(ProfileDataTypes::PROFILE, $profile_data['profile_id']);
        foreach ($user_data['fields'] as $field_id => $field_value) {
            if (array_key_exists($field_id, $profile)) {
                $user_data['fields'][$field_id] = $profile[$field_id];
                continue;
            }
            // profile field doesn't exist in this profile
            $user_data['fields'][$field_id] = '';
        }
        foreach ($profile as $field_id => $field_value) {
            if (array_key_exists($field_id, $user_data['fields'])) {
                continue;
            }
...
function fn_update_user($user_id, $user_data, &$auth, $ship_to_another, $notify_user)
{
    ...
    if (!empty($user_id)) {
        ...
        // 他の出品者商品を出品者データ管理者で購入した際の問題を修正
        ///////////////////////////////////////////////
        if (fn_allowed_for('MULTIVENDOR') && (AREA != 'A' || !isset($user_data['company_id']))) {
            $user_data['company_id'] = $current_user_data['company_id'];
        }
        ///////////////////////////////////////////////
...
function fn_update_user_profile($user_id, $user_data, $action = '', $ship_to_another = false)
{
    ...
    ///////////////////////////////////////////////////////////////
    fn_set_hook('jp_update_kana', $user_data);
    ///////////////////////////////////////////////////////////////
...
function fn_login_user($user_id = 0, $regenerate_session_id = false)
{
    ...
    if (!empty($user_id)) {
        ...
        if (fn_allowed_for('ULTIMATE')) {
            ///////////////////////////////////////////////////////////////
            // Modified for Japanese Ver by takahashi from cs-cart.jp 2018 BOF
            // 複数ショップで同じEメールのお客様が存在する場合に Auth API で見つからないバグを修正
            ///////////////////////////////////////////////////////////////
            if (Registry::get('settings.Stores.share_users') == 'N' && (AREA != 'A' || defined('API'))) {
            ///////////////////////////////////////////////////////////////
            // Modified for Japanese Ver by takahashi from cs-cart.jp 2018 EOF
            ///////////////////////////////////////////////////////////////
                $condition .= fn_get_company_condition('?:users.company_id');
            }
        }
...
function fn_recover_password_generate_key($user_email, $notify = true, $company_id = '', $area = AREA)
{
    ...
    if ($user_email) {
        ...
        if (fn_allowed_for('ULTIMATE')) {
            ...
            // 複数ショップで同じEメールのお客様が存在する場合に Auth API で見つからないバグを修正
            ///////////////////////////////////////////////////////////////
            } elseif (Registry::get('settings.Stores.share_users') === YesNo::NO && ($area !== SiteArea::ADMIN_PANEL || defined('API'))) {
            ///////////////////////////////////////////////////////////////
            // Modified for Japanese Ver by takahashi from cs-cart.jp 2018 EOF
            ///////////////////////////////////////////////////////////////
