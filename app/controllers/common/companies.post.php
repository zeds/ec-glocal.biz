...
if ($mode === 'get_companies_list') {
    ...
    if (empty($params['start']) && empty($params['pattern'])) {
        ...
        if (!empty($params['show_all']) && YesNo::toBool($params['show_all'])) {
            ...
            // 出品者指定ポップアップの表記について、管理画面ヘッダー部分のみ
            //「すべての出品者」を「マーケットプレイス管理モード」に変更
            ///////////////////////////////////////////////////////////////
            $all_vendors[0] = [
                'id'         => $is_search ? '' : 0,
                'company_id' => $is_search ? '' : 0,
                'value'      => $is_search ? '' : 0,
                'text'       => empty($params['default_label']) ? fn_lcjp_mve_convert_lang_var(__('all_vendors'), $params['result_ids'])  : __($params['default_label']),
                'name'       => empty($params['default_label']) ? fn_lcjp_mve_convert_lang_var(__('all_vendors'), $params['result_ids']) : __($params['default_label']),
                'append'     => '',
                'data'       => [
                    'id'         => $is_search ? '' : 0,
                    'company_id' => $is_search ? '' : 0,
