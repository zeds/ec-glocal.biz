...
****************************************************************************/
// Modified for mall by tommy from cs-cart.jp 2021
// パン屑リストにストア名を追加
// カテゴリーリンクをクリックした際に当該ストアの商品のみを抽出
// パン屑リストに商品名の代わりに「アイテム詳細」を表示
...
//
    ...
    /////////////////////////////////////////////////////////
    // Modified for mall by tommy from cs-cart.jp 2021 BOF
    // パン屑リストにストア名を追加
    // カテゴリーリンクをクリックした際に当該ストアの商品のみを抽出
    /////////////////////////////////////////////////////////
    if (fn_allowed_for('MULTIVENDOR:PLUS')) {
        ...
        if( !empty($product['company_id']) ){
            // ストアIDを変数にセット
            $store_id = $product['company_id'];
            // ストア名をパン屑リストに追加
            fn_add_breadcrumb($product['company_name'], "companies.products?company_id=$store_id");
            // 商品の抽出用ストアIDパラメータをセット
            $category_company_string = '&company_id=' . $store_id;
        // ストアIDが存在しない場合
        }else{
            // 商品の抽出用ストアIDパラメータに空データをセット
            $category_company_string = '';
        }
    }
    /////////////////////////////////////////////////////////
    // Modified for mall by tommy from cs-cart.jp 2021 EOF
    /////////////////////////////////////////////////////////
    ...
    if (!empty(Tygh::$app['session']['current_category_id'])) {
        ...
        if (!empty($parent_ids)) {
            ...
            foreach ($parent_ids as $c_id) {
                ...
                /////////////////////////////////////////////////////////
                if (fn_allowed_for('MULTIVENDOR:PLUS')) {
                    fn_add_breadcrumb($cats[$c_id], "categories.view?category_id=$c_id" . $category_company_string);
                }
                else{
    ...
    /////////////////////////////////////////////////////////
    if (fn_allowed_for('MULTIVENDOR:PLUS')) {
        fn_add_breadcrumb(__("jp_item_details"));
    }
    else{
