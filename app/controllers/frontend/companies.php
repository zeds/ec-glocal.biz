...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($mode === 'apply_for_vendor') {
        ...
        $account_data = [
            ...
            'admin_firstname' => isset($_REQUEST['company_data']['admin_lastname'])
                ? $_REQUEST['company_data']['admin_lastname']                : $company_data['admin_lastname'],
            'admin_lastname'  => isset($_REQUEST['company_data']['admin_firstname'])                ? $_REQUEST['company_data']['admin_firstname']                : $company_data['admin_firstname'],
        ];
        ///////////////////////////////////////////////
...
if ($mode === 'view') {
    ...
    if (fn_allowed_for('MULTIVENDOR:PLUS')) {
        fn_add_breadcrumb(__('jp_mall_store_list'), 'companies.catalog');
...
// 登録ストア一覧
    ...
    if (fn_allowed_for('MULTIVENDOR:PLUS')) {
        // お気に入りストア一覧の場合
        if( !empty($auth['user_id']) && $action == "fav_list"){
            // パン屑リストをセット
            fn_add_breadcrumb(__('jp_mall_store_fav_list'));
    
            // お気に入りストア判定フラグをSmarty変数にセット
            Tygh::$app['view']->assign('is_fav_stores', 'Y');
        // 通常のストア一覧の場合
        }else{
            // パン屑リストをセット
            fn_add_breadcrumb(__('jp_mall_store_list'));
        }
    }
    else{
