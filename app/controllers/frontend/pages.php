...
if ($mode == 'view') {
    ...
    if (fn_allowed_for('MULTIVENDOR:PLUS')) {
        // ストアIDが存在する場合
        if( !empty($page['company_id']) ){
            // ストアIDを変数にセット
            $store_id = $page['company_id'];
            $store_name = fn_get_company_name($store_id);
    
            if(!empty($store_name)){
                // ストア名をパン屑リストに追加
                fn_add_breadcrumb($store_name, "companies.products?company_id=$store_id");
            }
        }
    }
    /////////////////////////////////////////////////////////
