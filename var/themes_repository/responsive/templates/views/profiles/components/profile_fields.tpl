...
{/if}
    ...
    {else}  {* Simple input *}
        ...
        {* 電話番号と郵便番号を半角入力モードに変更 (type=tel) *}
        {if $smarty.const.CART_LANGUAGE == 'ja' && ($runtime.controller == 'profiles' || $runtime.controller == 'checkout' || $runtime.controller == 'companies') }
            {if $data_id == 'b_zipcode'}
                <input {if $field.autocomplete_type}x-autocompletetype="{$field.autocomplete_type}"{/if} type="tel" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}]" size="32" value="{$value}" class="ty-input-text{if ($field.autocomplete_type == "phone-full") || ($field.field_type == "ProfileFieldTypes::PHONE"|enum)} cm-mask-phone{/if} {if !$skip_field}{$_class}{else}cm-skip-avail-switch{/if} {if $smarty.foreach.profile_fields.index == 0} cm-focus{/if}" onKeyUp="AjaxZip3.zip2addr(this,'','user_data[b_state]','user_data[b_city]','','user_data[b_address]');" {if !$skip_field}{$disabled_param nofilter}{/if} />
            {elseif $data_id == 's_zipcode'}
                <input {if $field.autocomplete_type}x-autocompletetype="{$field.autocomplete_type}"{/if} type="tel" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}]" size="32" value="{$value}" class="ty-input-text{if ($field.autocomplete_type == "phone-full") || ($field.field_type == "ProfileFieldTypes::PHONE"|enum)} cm-mask-phone{/if} {if !$skip_field}{$_class}{else}cm-skip-avail-switch{/if} {if $smarty.foreach.profile_fields.index == 0} cm-focus{/if}"  onKeyUp="AjaxZip3.zip2addr(this,'','user_data[s_state]','user_data[s_city]','','user_data[s_address]');" {if !$skip_field}{$disabled_param nofilter}{/if} />
            {elseif $data_id == 'zipcode'}
                <input {if $field.autocomplete_type}x-autocompletetype="{$field.autocomplete_type}"{/if} type="tel" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}]" size="32" value="{$value}" class="ty-input-text{if ($field.autocomplete_type == "phone-full") || ($field.field_type == "ProfileFieldTypes::PHONE"|enum)} cm-mask-phone{/if} {if !$skip_field}{$_class}{else}cm-skip-avail-switch{/if} {if $smarty.foreach.profile_fields.index == 0} cm-focus{/if}" onKeyUp="AjaxZip3.zip2addr(this,'','company_data[state]','company_data[city]','','company_data[address]');" {if !$skip_field}{$disabled_param nofilter}{/if} />
            {else}
        <input
            {if $field.autocomplete_type}x-autocompletetype="{$field.autocomplete_type}"{/if}
            type={if ($field.field_type == "ProfileFieldTypes::PHONE"|enum || $data_id == "phone")}"tel"{else}"text"{/if}
            id="{$element_id}"
