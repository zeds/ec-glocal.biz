...
{* repay 画面で変数が取れない対応 BOF *}
{if $payment_method.processor_params}
    {assign var="p_method" value=$payment_method}
{else}
    {assign var="p_id" value=$payment_data.payment_id}
    {assign var="p_method" value=$p_id|fn_get_payment_method_data}
{/if}
{* repay 画面で変数が取れない対応 EOF *}
        {if $p_method.processor_params.use_cvv == 'true'}
        <div class="ty-credit-card__control-group ty-control-group">
        ...
        <div class="ty-credit-card__control-group ty-control-group">
            ...
            <select id="jp_cc_method" name="payment_info[jp_cc_method]" onchange="fn_check_smbc_cc_payment_type(this.value);">
                {if $p_method.processor_params.shiharai_kbn.1 == 'true'}
                    <option value="1">{__("jp_cc_onetime")}</option>
                {/if}
                {if $p_method.processor_params.shiharai_kbn.61 == 'true'}
                    <option value="61">{__("jp_cc_installment")}</option>
                {/if}
                {if $p_method.processor_params.shiharai_kbn.91 == 'true'}
                    <option value="91">{__("jp_cc_bonus_onetime")}</option>
                {/if}
                {if $p_method.processor_params.shiharai_kbn.80 == 'true'}
        ...
        <div class="ty-credit-card__control-group ty-control-group hidden" id="display_smbc_cc_splict_count">
            ...
            <select id="jp_cc_installment_times" name="payment_info[jp_cc_installment_times]">
                {foreach from=$p_method.processor_params.paycount item=paycount key=paycount_key name="paycounts"}
                    {if $p_method.processor_params.paycount.$paycount_key == 'true'}
        ...
        {if $p_method.processor_params.register_card_info == 'true' && $auth.user_id && $auth.user_id > 0}
        <div class="ty-credit-card__control-group ty-control-group">
    ...
    (function(_, $) {
        ...
        $.ceFormValidator('registerValidator', {
            ...
            func: function(id) {
                ...
                //テストモードの場合は常にTRUE
                {if $p_method.processor_params.mode == "test"}
        ...
        if(isFormVal && !isSubmit) {
            isSubmit = true;
                ...
                Fstokenizer().setConfig(
                    {
                        {* repay 画面で変数が取れない対応 BOF *}
                        shop_cd: '{$addons.smbc.shop_cd}',
                        syuno_co_cd: '{$addons.smbc.syuno_co_cd}',
                        {* repay 画面で変数が取れない対応 EOF *}
                        token: {
                            api_key: '{$p_method.processor_params.api_key}',
                            mode: 'S',
                            usecvv: {if $p_method.processor_params.use_cvv == 'true'}true{else}false{/if}
                        },
                        elems: {
            ...
            // repay 対応
            var repay_dispatch = document.getElementsByName('dispatch[orders.repay]')[0];
            ...
            if (smbc_token == false) {
                ...
                // repay 対応
                if(repay_dispatch){
                    checkoutForm.get(0).action = fn_url('orders.repay');
                }
