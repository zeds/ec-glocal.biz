...
<div class="clearfix cc_form_jp">
    <div class="ty-credit-card cm-cc_form_{$id_suffix}">
        ...
        {if $payment_method.processor_params.cust_manage == 'true' && $auth.user_id}
            <div class="ty-credit-card__control-group ty-control-group">
                <label for="use_uid" class="ty-control-group__title cm-required">{__('sbps_cc_register_card_info_use')}</label>
                <p>
                    <input type="radio" name="payment_info[cust_manage_flg]" value="1" checked="checked" class="radio" />{__('yes')}　
                    <input type="radio" name="payment_info[cust_manage_flg]" value="0" class="radio" />{__('no')}
                </p>
            </div>
        {/if}
...
<script type="text/javascript" src="{$token_js_url[$payment_method.processor_params.mode]}"></script>
<script type="text/javascript">
    ...
    Tygh.$.ceEvent('on', 'ce.commoninit', function(e_init) {
        ...
        const ap_sbps_dealings_type = document.getElementById('ap_sbps_dealings_type');
        ...
        if (cc_number && $(cc_number).is(':visible')) {
            ...
            if (ap_sbps_dealings_type) {
                ap_sbps_dealings_type.addEventListener('change', function(e) {
                    const container = document.getElementById('ap_sbps_divide_times_container');

                    if (e.target.value === '61') {
                        $(container).switchAvailability(false);
                    } else {
                        $(container).switchAvailability(true);
                    }
                });
            }
        }
    ...
    Tygh.$.ceFormValidator('registerValidator', {
        ...
        func: function(id) {
            ...
            //カードイメージの調整
            if(flag || ('{$payment_method.processor_params.mode}' === 'test' && input.value.length > 0)) {
    ...
    function submitHandler(e) {
        ...
        if (is_form_val && !is_submit) {
            ...
            com_sbps_system.generateToken({
                merchantId: '{$payment_method.processor_params.merchant_id}',
                serviceId: '{$payment_method.processor_params.service_id}',
