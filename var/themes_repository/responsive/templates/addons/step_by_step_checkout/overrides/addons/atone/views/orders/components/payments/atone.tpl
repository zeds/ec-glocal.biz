    	...
    	button.addEventListener(tap, function(e) {
    		...
    		{if $settings.Checkout.agree_terms_conditions == 'Y'}
    			...
    			if(!$(".cm-agreement").prop('checked')) {
    				var notification = '<div style="margin-right: 50px;" class="cm-auto-hide cm-notification-content notification-content alert alert-warning"><button type="button" class="close cm-notification-close " data-dismiss="alert">×</button><strong>' + warning + '</strong>' + agreement_alert + '</div>';
