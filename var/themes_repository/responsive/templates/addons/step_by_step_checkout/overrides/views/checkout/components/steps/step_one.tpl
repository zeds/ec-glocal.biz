        ...
        {else}
            {if $auth.user_id != 0}
                {if $user_data.firstname || $user_data.lastname}
                        ...
                        {if $smarty.const.CART_LANGUAGE == "ja" && $user_data.firstname}
                            {assign var="dear" value=__("dear")}
                            {assign var="login_info" value="`$user_data.firstname`&nbsp;`$user_data.lastname`&nbsp;`$dear`"}
                        {else}
            ...
            <div id="step_one_register" class="clearfix{if $smarty.request.login_type != "register"} hidden{/if}">
                <form name="step_one_register_form" class="{$ajax_form} cm-ajax-full-render" action="{""|fn_url}" method="post">
                    ...
                    <div class="checkout__block">
                        ...
                        {* Modified by tommy from cs-cart.jp 2017 *}
                        {hook name="auth:jp_login_form_pre"}{/hook}
