...
<div class="control-group profile-field-wrap {$field.class}">
    ...
    {else}  {* Simple input *}
        {if $smarty.const.CART_LANGUAGE == 'ja' && ($runtime.controller == 'profiles' || $runtime.controller == 'checkout') }
            {if $data_id == 'b_zipcode'}
                <input {if $field.autocomplete_type}x-autocompletetype="{$field.autocomplete_type}"{/if} type="text" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}]" size="32" value="{$value}" class="input-text {if !$skip_field}{$_class}{else}cm-skip-avail-switch{/if}" onKeyUp="AjaxZip3.zip2addr(this,'','user_data[b_state]','user_data[b_city]','','user_data[b_address]');" {if !$skip_field}{$disabled_param}{/if} />
            {elseif $data_id == 's_zipcode'}
                <input {if $field.autocomplete_type}x-autocompletetype="{$field.autocomplete_type}"{/if} type="text" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}]" size="32" value="{$value}" class="input-text {if !$skip_field}{$_class}{else}cm-skip-avail-switch{/if}" onKeyUp="AjaxZip3.zip2addr(this,'','user_data[s_state]','user_data[s_city]','','user_data[s_address]');" {if !$skip_field}{$disabled_param}{/if} />
            {else}
