    ...
    public function setupLanguages($demo)
    {
        ...
        if (!empty($languages)) {
            ...
            foreach ($languages as $lang_code) {
                ...
                if($lang_code == 'ja') $flg_jp_selected = true;
            ...
            if(!$flg_jp_selected){
                $this->_parseSql(Registry::get('config.dir.install') . App::DB_LANG . "/ja/" . App::DB_LANG_DATA, 'text_installing_additional_language', array('lang_code' => 'ja'));
                if ($demo) {
                    $this->_parseSql(Registry::get('config.dir.install') . App::DB_LANG . "/ja/" . App::DB_LANG_DEMO, 'text_installing_additional_language', array('lang_code' => 'ja'));
                }

                // Install language variables from PO files
                $pack_path = Registry::get('config.dir.install') . App::DB_LANG . "/ja/ja.po";
                $edition_pack_path = Registry::get('config.dir.install') . App::DB_LANG . "/ja/ja" . '_' . fn_get_edition_acronym(PRODUCT_EDITION) . '.po';
