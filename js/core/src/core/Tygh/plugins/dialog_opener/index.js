...
export const methods = {
    ...
    reload: function () {
        ...
        if ($(this).dialog('option', 'destroyOnClose') === false) {
            d.data('isReloadingDialog', true);
            d.dialog('close');
            d.dialog('open');
            d.removeData('isReloadingDialog');
        } else {
            d.ceDialog('resize');
    ...
    _init: function (params) {
        ...
        container.dialog({
            ...
            open: function (e, u) {
                ...
                if (stack.length) {
                    var prev        = stack.pop(),
                        $prevParent = $('#' + prev).parent('.ui-front');

                    stack.push(prev);
                    _zindex = $prevParent.css('z-index') ? $prevParent.css('z-index') : 0;
                }

                w.css('z-index', ++_zindex);
                w.prev().css('z-index', _zindex);
            ...
            close: function (e, u) {
                ...
                // 管理画面で注文作成時にカテゴリーが選択されない問題を修正
                if ($(this).dialog('option', 'destroyNestedOnClose')) {
                    $('.cm-dialog-opener', this).each(function () {
                        var $nestedDialog = $('#'  $(this).data('caTargetId'));
                        if (!$nestedDialog.length
                            || !$nestedDialog.data('uiDialog')
                        ) {
                            return;
                        }

                        $nestedDialog.dialog('destroy').remove();
                    });
                }
...
export const ceDialogInit = function ($) {
    ...
    $.ceDialog = function (action, params) {
        ...
        if (action == 'get_last') {
            ...
            // Modified by takahashi from cs-cart.jp 2022 BOF
            // 管理画面で注文作成時にカテゴリーが選択されない問題を修正
            if (params.hasClass('cm-dialog-destroy-nested-on-close')) {
                dialog_params['destroyNestedOnClose'] = true;
            }
            // Modified by takahashi from cs-cart.jp 2022 EOF
