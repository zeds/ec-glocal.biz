...
(function (_, $) {
  ...
  var SHIPPING_PREFIX = 's_',
      BILLING_PREFIX = 'b_',
      NAME_SEPARATOR = ' ',
      NAME_SEPARATOR_JP = '　', // Modified by takahashi from cs-cart.jp 2019
  ...
  var methods = {
    ...
    splitFullname: function splitFullname(fullname, fullnameFormat) {
      ...
      if (fullnameFormat === 'firstname_first') {
          if (lastname == '') {
              nameParts = fullname.split(NAME_SEPARATOR_JP).map(function (part) {
                  return part.trim();
              }).filter(function (part) {
                  return part !== '';
              }),
                  lastname = nameParts.pop(),
                  firstname = nameParts.join(NAME_SEPARATOR_JP);
          }
      }
      else {
          if (firstname == '') {
  ...
  $(_.doc).on('click', '.cm-open-pickups', function (e) {
    ...
    /* Modified by takahashi from cs-cart.jp 2019 BOF */
    $(_.doc).on('change', '.jp_zipcode_change', function (e) {
        methods.lockShippingMethodSelector();
    });
    /* Modified by takahashi from cs-cart.jp 2019 EOF */
