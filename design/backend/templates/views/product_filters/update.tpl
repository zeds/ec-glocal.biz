            ...
            {if !$id}
                ...
                {if $filter_fields}
                    <optgroup label="{__("product_fields")}">
                        {foreach $filter_fields as $field_type => $field}
                            {if !$field.hidden}
...
{* Modified by tommy from cs-cart.jp 2017 BOF *}
                ...
                <option value="1000"{if $filter.round_to == 1000} selected="selected"{/if}>1000</option>
                <option value="10000"{if $filter.round_to == 10000} selected="selected"{/if}>10000</option>
...
{if $in_popup}
    <div class="buttons-container">
        ...
        {/if}
        {include file="buttons/save_cancel.tpl" but_name="dispatch[product_filters.update]" cancel_action="close" hide_first_button=$hide_first_button save=$id cancel_meta="bulkedit-unchanged"}
    </div>
{else}
