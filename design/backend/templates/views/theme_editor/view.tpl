...
<div class="theme-editor {if $theme_manifest.converted_to_css} te-converted-to-css{/if}" data-ca-te-use-dynamic-style="{if $runtime.vendor_id}true{else}false{/if}" id="theme_editor_container" data-bp-sidebar="true">
    ...
    <div class="te-header{if !$props_schema} te-header-no-schema{/if}" id="te_styles_list">
        ...
        <span class="te-title">
            ...
            {__("theme_editor")}
            {if !$theme_manifest.converted_to_css && !$runtime.vendor_id}
...
{* Modified by takahashi from cs-cart.jp 2018 BOF *}
                    <button class="te-btn-action ty-float-right jp-te-btn-save" type="submit" name="dispatch[theme_editor.save]">{__("save")}</button>
