    ...
    {capture name="carts_table"}
        <div class="table-responsive-wrapper longtap-selection">
            ...
            >
                <tr>
                    <th width="6%">
                        {include file="common/check_items.tpl" is_check_all_shown=true is_check_disabled=!$has_permission_update}
            ...
            >
                <td width="6%" class="left mobile-hide">
                    {if "ULTIMATE"|fn_allowed_for}
                        <input type="checkbox" name="user_ids[{$customer.company_id}][]" value="{$customer.user_id}" class="cm-item hide" /></td>
                    {/if}
                    {if !"ULTIMATE"|fn_allowed_for}
                        <input type="checkbox" name="user_ids[]" value="{$customer.user_id}" class="cm-item hide" /></td>
                        {if $customer.storefront_id}
                            <input type="hidden" name="storefront_id" value="{$customer.storefront_id}" class="cm-item" /></td>
                        {/if}
                    {/if}
                <td width="45%" data-th="{__("customer")}">
                    <div class="cart__customer">
                        ...
                        <div class="cart__customer-data-wrapper">
                            ...
                            {if $customer.user_data.email}
                                    ...
                                    {if $customer.user_data.user_id}<a href="{"profiles.update?user_id=`$customer.user_id`"|fn_url}">{/if}
                                    ...
                                    {if $customer.user_data.user_id}</a>{/if}
                                {else}
                ...
                <td width="8%" class="right" data-th="{__("total")}">
