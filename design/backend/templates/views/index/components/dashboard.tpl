        ...
        _.drawChart = function(is_day) {
            ...
            function get_data(div) {
                ...
                var date_formatter = new google.visualization.DateFormat({
                    pattern: "{__("current_previous_period", ["[date]" => "MMM d, yyyy"])|escape:"javascript"}"
                });
...
{hook name="index:index"}
    ...
    <div class="dashboard row-fluid" id="dashboard">
        ...
        <div class="dashboard-cards span3">
            ...
            {hook name="index:products_statistic"}
                                    ...
                                    <table class="table table-middle table--relative table-last-td-align-right table-responsive table-responsive-w-titles">
                                        ...
                                        {foreach $orders.all as $order}
                                            <tr>
                                                ...
                                                <td data-th="&nbsp;"><a href="{"orders.details?order_id=`$order.order_id`"|fn_url}">{__("order")} <bdi>#{$order.order_id}</bdi></a> {__("by")} {if $order.user_id}<a href="{"profiles.update?user_id=`$order.user_id`"|fn_url}">{/if}{* Modified by takahashi from cs-cart.jp 2020 BOF *}{if $smarty.const.CART_LANGUAGE == "ja"}{$order.firstname} {$order.lastname}{else}{$order.lastname} {$order.firstname}{/if}{* Modified by takahashi from cs-cart.jp 2020 EOF *}{if $order.user_id}</a>{/if}</td>
...
{capture name="buttons"}
    {include file="common/daterange_picker.tpl" id="dashboard_date_picker" extra_class="pull-right" data_url="index.index?storefront_id={$storefront_id}"|fn_url result_ids="dashboard" start_date=$time_from end_date=$time_to}
{/capture}
