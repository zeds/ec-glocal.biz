...
{capture name="mainbox"}
    ...
    {capture name="sidebar"}
        {include file="addons/kuroneko_webcollect/views/krnkwc_cc_manager/components/orders_search_form.tpl" dispatch="krnkwc_cc_manager.manage"}
    {/capture}
    ...
    <form action="{""|fn_url}" method="post" target="_self" name="krnkwc_cc_orders_list_form">
        ...
        {assign var="page_title" value=__("jp_kuroneko_webcollect_cc_order_status_long")}
        ...
        <div class="table-wrapper">
            <table width="100%" class="table table-middle">
                ...
                <tr>
                    ...
                    <th width="15%"><a class="cm-ajax" href="{"`$c_url`&sort_by=krnk_status_code&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("jp_kuroneko_webcollect_cc_status")}{if $search.sort_by == "krnk_status_code"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                ...
                {foreach from=$orders item="o"}
                    <tr>
                        ...
                        <td>{$o.krnk_status_code|fn_krnkwc_get_cc_status_name}</td>
                        ...
                        <td width="5%" class="center">
                            {capture name="tools_items"}
                                ...
                                <li><a class="cm-confirm" href="{"krnkwc_cc_manager.wc_check_status?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_kuroneko_webcollect_check_wc_order_status")}</a></li>
                                {if $o.krnk_status_code|fn_krnkwc_is_wc_processable:"creditcancel"}
                                    <li><a class="cm-confirm" href="{"krnkwc_cc_manager.creditcancel?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_kuroneko_webcollect_cc_auth_cancel")}</a></li>
                                {/if}
                            {/capture}
...
{capture name="buttons"}
    {if $orders}
        {capture name="tools_list"}
            <li>{btn type="list" text={__("jp_kuroneko_webcollect_cc_auth_cancel")} dispatch="dispatch[krnkwc_cc_manager.bulk_creditcancel]" form="krnkwc_cc_orders_list_form" class="cm-process-items cm-confirm"}</li>
        {/capture}
