...
{capture name="mainbox"}
    ...
    {capture name="sidebar"}
        {include file="addons/payjp/views/payjp_cc_manager/components/orders_search_form.tpl" dispatch="payjp_cc_manager.manage"}
    {/capture}

    <form action="{""|fn_url}" method="post" target="_self" name="payjp_orders_list_form">
        ...
        <div class="table-wrapper">
            <table width="100%" class="table table-middle">
                ...
                <tr>
                    ...
                    <th width="15%"><a class="cm-ajax" href="{"`$c_url`&sort_by=cc_status&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("jp_payjp_cc_status")}{if $search.sort_by == "cc_status"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                ...
                {foreach from=$orders item="o"}
                    <tr>
                        ...
                        <td>{$o.cc_status_code|fn_payjp_get_cc_status_name}</td>
                        ...
                        <td width="5%" class="center">
                            {capture name="tools_items"}
                                ...
                                {if $o.cc_status_code == 'AUTH_OK'}
                                    <li><a class="cm-confirm" href="{"payjp_cc_manager.cc_sales_confirm?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_payjp_cc_sales_confirm")}</a></li>
                                    <li><a class="cm-confirm" href="{"payjp_cc_manager.cc_auth_cancel?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_payjp_cc_auth_cancel")}</a></li>
                                {elseif $o.cc_status_code == 'SALES_CONFIRMED' || $o.cc_status_code == 'CAPTURE_OK'}
                                    <li><a class="cm-confirm" href="{"payjp_cc_manager.cc_sales_cancel?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_payjp_cc_sales_cancel")}</a></li>
                                {/if}
                            {/capture}
...
{capture name="buttons"}
    {if $orders}
        {capture name="tools_list"}
            <li>{btn type="list" text={__("jp_payjp_cc_sales_confirm")} dispatch="dispatch[payjp_cc_manager.bulk_sales_confirm]" form="payjp_orders_list_form" class="cm-process-items cm-confirm"}</li>
            <li>{btn type="list" text={__("jp_payjp_cc_auth_cancel")} dispatch="dispatch[payjp_cc_manager.bulk_auth_cancel]" form="payjp_orders_list_form" class="cm-process-items cm-confirm"}</li>
            <li>{btn type="list" text={__("jp_payjp_cc_sales_cancel")} dispatch="dispatch[payjp_cc_manager.bulk_sales_cancel]" form="payjp_orders_list_form" class="cm-process-items cm-confirm"}</li>
        {/capture}
