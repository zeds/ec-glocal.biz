...
{* gmo_deferred_payment start *}
	{assign var="gmo_order" value=$order_info.order_id|fn_gmo_deferred_payment_order}
{* gmo_deferred_payment end *}
...
<div class="cm-tabs-content" id="tabs_content">
    <div id="content_tab_general">
    	{* gmo_deferred_payment start *}
		{if $gmo_order}
        	<p>{__("gmo_delivery_management")}</p>
        {/if}
        {* gmo_deferred_payment end *}
...
{if $gmo_order}
	...
	<script type="text/javascript">
		(function(_, $) {        
  			$("[id *= 'shipment_data_']").prop('disabled', true);
  			$('form').bind('submit', function () {
    			$(this).find("[id *= 'shipment_data_']").prop('disabled', false);
  			});
		}(Tygh, Tygh.$));
	</script>
	{/literal}
