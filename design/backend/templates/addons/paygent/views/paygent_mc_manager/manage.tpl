...
{capture name="mainbox"}
    ...
    {capture name="sidebar"}
        {include file="addons/paygent/views/paygent_mc_manager/components/orders_search_form.tpl" dispatch="paygent_mc_manager.manage"}
    {/capture}
    ...
    <form action="{""|fn_url}" method="post" target="_self" name="paygent_orders_list_form">
        ...
        {else}
            {assign var="page_title" value=__("jp_paygent_service_name_mc")}
        ...
        <div class="table-wrapper">
            <table width="100%" class="table table-middle">
                ...
                <tr>
                    ...
                    <th width="15%"><a class="cm-ajax" href="{"`$c_url`&sort_by=cc_status&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("jp_paygent_mc_status")}{if $search.sort_by == "cc_status"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                        ...
                        <td width="5%" class="center">
                            {capture name="tools_items"}
                                ...
                                {if $o.cc_status_code == 'AUTH_OK'}
                                    <li><a class="cm-confirm" href="{"paygent_mc_manager.mc_sales_confirm?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_paygent_mc_sales_confirm")}</a></li>
                                    <li><a class="cm-confirm" href="{"paygent_mc_manager.mc_auth_cancel?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_paygent_mc_auth_cancel")}</a></li>
                                {elseif $o.cc_status_code == 'SALES_CONFIRMED' || $o.cc_status_code == 'CAPTURE_OK'}
                                    <li><a class="cm-confirm" href="{"paygent_mc_manager.mc_sales_cancel?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_paygent_mc_sales_cancel")}</a></li>
                                {/if}
                            {/capture}
...
{capture name="buttons"}
    {if $orders}
        {capture name="tools_list"}
            <li>{btn type="list" text={__("jp_paygent_mc_sales_confirm")} dispatch="dispatch[paygent_mc_manager.bulk_sales_confirm]" form="paygent_orders_list_form" class="cm-process-items cm-confirm"}</li>
            <li>{btn type="list" text={__("jp_paygent_mc_auth_cancel")} dispatch="dispatch[paygent_mc_manager.bulk_auth_cancel]" form="paygent_orders_list_form" class="cm-process-items cm-confirm"}</li>
            <li>{btn type="list" text={__("jp_paygent_mc_sales_cancel")} dispatch="dispatch[paygent_mc_manager.bulk_sales_cancel]" form="paygent_orders_list_form" class="cm-process-items cm-confirm"}</li>
        {/capture}
