...
{capture name="mainbox"}
    ...
    {capture name="sidebar"}
        {include file="addons/kuroneko_kakebarai/views/krnkkb_manager/components/orders_search_form.tpl" dispatch="krnkkb_manager.manage"}
    {/capture}

    <form action="{""|fn_url}" method="post" target="_self" name="krnkkb_orders_list_form">
        ...
        {assign var="page_title" value=__("jp_kuroneko_kakebarai_order_status_long")}
        ...
        {if $orders}
            <table width="100%" class="table table-middle">
                ...
                <tr>
                    ...
                    <th width="15%"><a class="cm-ajax" href="{"`$c_url`&sort_by=krnk_status_code&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("jp_kuroneko_kakebarai_order_status")}{if $search.sort_by == "krnk_status_code"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                ...
                {foreach from=$orders item="o"}
                    <tr>
                        ...
                        <td>{$o.krnk_status_code|fn_krnkkb_get_status_name}</td>
                        ...
                        <td width="5%" class="center">
                            {capture name="tools_items"}
                                ...
                                {if $o.krnk_status_code == 'KB_1'}
                                <li><a class="cm-confirm" href="{"krnkkb_manager.kb_cancel?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("jp_kuroneko_kakebarai_cancel")}</a></li>
                                {/if}
                            {/capture}
...
{capture name="buttons"}
    {if $orders}
        {capture name="tools_list"}
            <li>{btn type="list" text={__("jp_kuroneko_kakebarai_cancel")} dispatch="dispatch[krnkkb_manager.bulk_kb_cancel]" form="krnkkb_orders_list_form" class="cm-process-items cm-confirm"}</li>
        {/capture}
