{capture name='mainbox'}
    ...
    {capture name='sidebar'}
        {include file='addons/ap_sbps/views/ap_sbps_rb_career_manager/components/orders_search_form.tpl' dispatch='ap_sbps_rb_career_manager.manage'}
    {/capture}

    {include file='addons/ap_sbps/components/ap_sbps_rb_manager_tabs.tpl'}
            ...
            <div class="table-responsive-wrapper">
                <table width="100%" class="table table-middle table-responsive">
                    ...
                    <tr>
                        ...
                        <th width="10%"><a class="cm-ajax" href="{"`$c_url`&sort_by=order_id&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__('id')}{if $search.sort_by == 'order_id'}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                        ...
                        <th width="8%"><a class="cm-ajax" href="{"`$c_url`&sort_by=is_charge&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__('sbps_is_charge')}{if $search.sort_by == 'is_charge'}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                        <th width="10%"><a class="cm-ajax" href="{"`$c_url`&sort_by=refunded_at&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__('sbps_refunded_at')}{if $search.sort_by == 'refunded_at'}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                        <th width="15%"><a class="cm-ajax" href="{"`$c_url`&sort_by=canceled_at&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__('sbps_canceled_at')}{if $search.sort_by == 'canceled_at'}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                        ...
                        <th width="10%" class="center"><a class="cm-ajax" href="{"`$c_url`&sort_by=customer&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__('customer')}{if $search.sort_by == 'customer'}{$c_icon nofilter}{/if}</a></th>
                        <th>&nbsp;</th>
                        <th width="15%" class="right"><a class="cm-ajax{if $search.sort_by == 'total'} sort-link-{$search.sort_order_rev}{/if}" href="{"`$c_url`&sort_by=total&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__('total')}</a></th>
                    </tr>
                    </thead>
                    {foreach from=$orders item='o'}
                        <tr>
                            ...
                            <td data-th="{__('sbps_is_charge')}">{__("sbps_is_charge_`$o.is_charge`")}</td>
                            <td data-th="{__('sbps_refunded_at')}">
                                {if $o.sbps_refunded_at}
                                    {$o.sbps_refunded_at|date_format:'Y/m'}
                                {/if}
                            </td>
                            <td data-th="{__('sbps_canceled_at')}">
                                {if $o.canceled_at}
                                    {$o.canceled_at|date_format:"`$settings.Appearance.date_format`, `$settings.Appearance.time_format`"}
                                {/if}
                            </td>
                                ...
                                {capture name='tools_items'}
                                    ...
                                    {foreach from=$exec_permission[$o.pay_method]['status'][$o.payment_status] key=key item=mode}
                                        {if $mode|substr:0:1 != '_'}
                                            <li><a class="cm-confirm" href="{"ap_sbps_rb_career_manager.`$mode`?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("sbps_rb_`$mode`")}</a></li>
                                        {/if}
                                    {/foreach}
                                    {if $o.is_charge == '1'}
                                        {foreach from=$exec_permission[$o.pay_method]['charge'] key=key item=mode}
                                            {if $mode|substr:0:1 != '_'}
                                                <li><a class="cm-confirm" href="{"ap_sbps_rb_career_manager.`$mode`?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("sbps_rb_`$mode`")}</a></li>
                                            {/if}
                                        {/foreach}
...
{capture name='buttons'}
    {if $orders}
        {capture name='tools_list'}
            <li>{btn type='list' text={__('sbps_rb_refund')} dispatch='dispatch[ap_sbps_rb_career_manager.refund]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
        {/capture}
