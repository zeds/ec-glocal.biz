{capture name='mainbox'}
    ...
    {capture name='sidebar'}
        {include file='addons/ap_sbps/views/ap_sbps_rp_career_manager/components/orders_search_form.tpl' dispatch='ap_sbps_rp_career_manager.manage'}
    {/capture}
                                ...
                                {capture name='tools_items'}
                                    ...
                                    {foreach from=$exec_permission[$o.pay_method]['status'][$o.payment_status] key=key item=mode}
                                        {if $mode|substr:0:1 != '_'}
                                            <li><a class="cm-confirm" href="{"ap_sbps_rp_career_manager.`$mode`?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("sbps_rp_`$mode`")}</a></li>
                                        {/if}
                                    {/foreach}
                                    {if $o.is_canceled == '0'}
...
{capture name='buttons'}
    {if $orders}
        {capture name='tools_list'}
            <li>{btn type='list' text={__('sbps_rp_sales_confirm')} dispatch='dispatch[ap_sbps_rp_career_manager.sales_confirm]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
            <li>{btn type='list' text={__('sbps_rp_cancel')} dispatch='dispatch[ap_sbps_rp_career_manager.cancel]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
        {/capture}
