{capture name='mainbox'}
    ...
    {capture name='sidebar'}
        {include file='addons/ap_sbps/views/ap_sbps_rp_credit_manager/components/orders_search_form.tpl' dispatch='ap_sbps_rp_credit_manager.manage'}
    {/capture}

    {include file='addons/ap_sbps/components/ap_sbps_rp_manager_tabs.tpl'}
                                ...
                                {capture name='tools_items'}
                                    ...
                                    {foreach from=$exec_permission[$o.pay_method]['status'][$o.payment_status] key=key item=mode}
                                        {if $mode|substr:0:1 != '_'}
                                            <li><a class="cm-confirm" href="{"ap_sbps_rp_credit_manager.`$mode`?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("sbps_rp_`$mode`")}</a></li>
                                        {/if}
                                    {/foreach}
                                    {if $o.is_canceled == '0'}
                                        {foreach from=$exec_permission[$o.pay_method]['purchase'] key=key item=mode}
                                            {if $mode|substr:0:1 != '_'}
                                                <li><a class="cm-confirm" href="{"ap_sbps_rp_credit_manager.`$mode`?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("sbps_rp_`$mode`")}</a></li>
                                            {/if}
                                        {/foreach}
...
{capture name='buttons'}
    {if $orders}
        {capture name='tools_list'}
            <li>{btn type='list' text={__('sbps_rp_sales_confirm')} dispatch='dispatch[ap_sbps_rp_credit_manager.sales_confirm]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
            <li>{btn type='list' text={__('sbps_rp_cancel')} dispatch='dispatch[ap_sbps_rp_credit_manager.cancel]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
            {if $view_type == 'A'}
                <li>{btn type='list' text={__('sbps_rp_update_payment_info')} dispatch='dispatch[ap_sbps_rp_credit_manager.update_payment_info]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
            {/if}
        {/capture}
