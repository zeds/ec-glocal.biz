{capture name='mainbox'}
    ...
    {capture name='sidebar'}
        {include file='addons/ap_sbps/views/ap_sbps_rb_credit_manager/components/orders_search_form.tpl' dispatch='ap_sbps_rb_credit_manager.manage'}
    {/capture}
            ...
            <div class="table-responsive-wrapper">
                <table width="100%" class="table table-middle table-responsive">
                    ...
                    <tr>
                        ...
                        <th width="10%"><a class="cm-ajax" href="{"`$c_url`&sort_by=is_charge&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__('sbps_is_charge')}{if $search.sort_by == 'is_charge'}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
                                ...
                                {capture name='tools_items'}
                                    ...
                                    {foreach from=$exec_permission[$o.pay_method]['status'][$o.payment_status] key=key item=mode}
                                        {if $mode|substr:0:1 != '_'}
                                            <li><a class="cm-confirm" href="{"ap_sbps_rb_credit_manager.`$mode`?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("sbps_rb_`$mode`")}</a></li>
                                        {/if}
                                    {/foreach}
                                    {if $o.is_charge == '1'}
