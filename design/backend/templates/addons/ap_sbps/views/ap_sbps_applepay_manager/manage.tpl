{capture name='mainbox'}
    ...
    {capture name='sidebar'}
        {include file='addons/ap_sbps/views/ap_sbps_applepay_manager/components/orders_search_form.tpl' dispatch='ap_sbps_applepay_manager.manage'}
    {/capture}
                        ...
                        <tr>
                            ...
                            <td width="5%" class="center" data-th="{__('tools')}">
                                {capture name='tools_items'}
                                    ...
                                    {foreach from=$exec_permission['applepaypay']['status'][$o.payment_status] key=key item=mode}
                                        {if $mode|substr:0:1 != '_'}
                                            <li><a class="cm-confirm" href="{"ap_sbps_applepay_manager.`$mode`?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("sbps_`$mode`")}</a></li>
                                        {/if}
                                    {/foreach}
                                    {foreach from=$exec_permission['applepaypay']['all'] key=key item=mode}
                                        {if $mode|substr:0:1 != '_'}
                                            <li><a class="cm-confirm" href="{"ap_sbps_applepay_manager.`$mode`?order_id=`$o.order_id`&amp;redirect_url=`$current_redirect_url`"|fn_url}">{__("sbps_`$mode`")}</a></li>
                                        {/if}
                                    {/foreach}
...
{capture name='buttons'}
    {if $orders}
        {capture name='tools_list'}
            <li>{btn type='list' text={__('sbps_sales_confirm')} dispatch='dispatch[ap_sbps_applepay_manager.sales_confirm]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
            <li>{btn type='list' text={__('sbps_cancel')} dispatch='dispatch[ap_sbps_applepay_manager.cancel]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
            <li>{btn type='list' text={__('sbps_update_payment_info')} dispatch='dispatch[ap_sbps_applepay_manager.update_payment_info]' form='ap_sbps_orders_list_form' class='cm-process-items cm-confirm cm-submit'}</li>
        {/capture}
