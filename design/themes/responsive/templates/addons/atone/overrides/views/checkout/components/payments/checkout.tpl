        ...
        <div class="litecheckout__shipping-method litecheckout__field litecheckout__field--xsmall">
			...
			{if $payment.processor_id == '9088'}
            	<input type="radio"
                   ...
                   class="atone_radio litecheckout__shipping-method__radio cm-select-payment{if $payment|fn_lcjp_check_payment}_submit{/if} hidden"
                   {* Modified by takahashi from cs-cart.jp 2019 EOF *}
                   value="{$payment.payment_id}"
                   {if $payment.payment_id == $cart.payment_id && !$smarty.request.full_render}checked{/if}
            	/>
            	...
            	<script>
					...
					(function (_, $) {
						...
						$('.litecheckout__field input, .litecheckout__field select').focusin(function(e) {
      						$('.atone_radio').prop('checked', false);
      						$('#atone-button').addClass("atone-disabled");
    					});
    					
    					$('.cm-switch-availability').click(function(){
    						$('.atone_radio').prop('checked', false);
      						$('#atone-button').addClass("atone-disabled");
    					});
            ...
            {else}
            	<input type="radio"
                   ...
                   data-ca-result-ids="litecheckout_final_section,litecheckout_step_payment,shipping_rates_list,checkout*"
                   {* Modified by takahashi from cs-cart.jp 2019 BOF *}
                   class="litecheckout__shipping-method__radio cm-select-payment{if $payment|fn_lcjp_check_payment}_submit{/if} hidden"
                   {* Modified by takahashi from cs-cart.jp 2019 EOF *}
                   value="{$payment.payment_id}"
