...
{if $show_price_values}
    {if $settings.Appearance.show_prices_taxed_clean == "Y" && $product.taxed_price}
        ...
        {if $product.clean_price != $product.taxed_price && $product.included_tax}
            ...
            {assign var="jp_disable_you_save" value=true}

            {* 割引前価格または通常価格を表示する設定の場合 *}
            {if $show_old_price}
                {* 商品またはカテゴリー単位の割引が設定されている場合 *}
                {if $product.discount}
                    {* 割引前価格に税込金額をセット *}
                    {assign var="jp_original_price_for_display" value=$product.taxed_original_price}
                {* 通常価格が設定されている場合 *}
                {elseif $product.list_discount}
                    {* 通常価格に税込金額をセット *}
                    {assign var="jp_list_price_for_display" value=$product.taxed_list_price}
                {/if}
                {* 金額の後に（税込）表記を追加 *}
                {assign var="jp_old_price_tax_desc" value=__("including_tax")}
            {/if}
        {* 内税の場合 *}
        {else}
            {* 表示価格に税込金額をセット *}
            ...
            {if $show_old_price}
                ...
                {if $product.discount}
                    {* 通常価格に税込金額をセット *}
                    {assign var="jp_original_price_for_display" value=$product.original_price|default:$product.base_price}
                {* 通常価格が設定されている場合 *}
                {elseif $product.list_discount}
                    {* 通常価格に税込金額をセット *}
                    {assign var="jp_list_price_for_display" value=$product.list_price}
                {/if}
            {/if}
        {/if}
    {* 税金の設定がない場合 *}
...
{capture name="old_price_`$obj_id`"}
    {if $show_price_values && $show_old_price}
        <span class="cm-reload-{$obj_prefix}{$obj_id}" id="old_price_update_{$obj_prefix}{$obj_id}">
            ...
            {* Modified by tommy from cs-cart.jp 2017 BOF *}
            {* 商品・カテゴリー単位での割引が適用されている場合 *}
            {if $product.discount}
                <span class="ty-list-price ty-nowrap" id="line_old_price_{$obj_prefix}{$obj_id}">{if $details_page}{__("old_price")}: {/if}<span class="ty-strike">{include file="common/price.tpl" value=$jp_original_price_for_display span_id="old_price_`$obj_prefix``$obj_id`" class="ty-list-price ty-nowrap"}{if $jp_old_price_tax_desc} ({$jp_old_price_tax_desc}){/if}</span></span>
            {* 通常価格が設定されている場合 *}
            {elseif $product.list_discount}
                <span class="ty-list-price ty-nowrap" id="line_list_price_{$obj_prefix}{$obj_id}">{if $details_page}<span class="list-price-label">{__("list_price")}:</span> {/if}<span class="ty-strike">{include file="common/price.tpl" value=$jp_list_price_for_display span_id="list_price_`$obj_prefix``$obj_id`" class="ty-list-price ty-nowrap"}{if $jp_old_price_tax_desc} ({$jp_old_price_tax_desc}){/if}</span></span>
            {/if}
            {* Modified by tommy from cs-cart.jp 2017 EOF *}
            {/hook}
        ...
        {if $show_price_values}
            ...
            {hook name="products:prices_block"}
                {if $product.price|floatval || $product.zero_price_action == "P" || ($hide_add_to_cart_button == "Y" && $product.zero_price_action == "A")}
                    {* Modified by tommy from cs-cart.jp 2017 BOF *}
                    <span class="ty-price{if !$product.price|floatval && !$product.zero_price_action} hidden{/if}" id="line_discounted_price_{$obj_prefix}{$obj_id}">{if $details_page}{/if}{include file="common/price.tpl" value=$jp_price_for_display span_id="discounted_price_`$obj_prefix``$obj_id`" class="ty-price-num"}{if $product.clean_price != $product.taxed_price}<span class="ty-price-num"> ({__("including_tax")})</span>{/if}</span>
                    {* Modified by tommy from cs-cart.jp 2017 EOF *}
...
{capture name="clean_price_`$obj_id`"}
    {if $show_price_values && $show_clean_price && $settings.Appearance.show_prices_taxed_clean == "Y" && $product.taxed_price}
        <span class="cm-reload-{$obj_prefix}{$obj_id}" id="clean_price_update_{$obj_prefix}{$obj_id}">
            ...
            {if $product.clean_price != $product.taxed_price && $product.included_tax}
                {* Modified by tommy from cs-cart.jp 2017 BOF *}
                <span class="ty-list-price ty-nowrap" id="line_product_price_{$obj_prefix}{$obj_id}">({__("jp_exc_tax")} {include file="common/price.tpl" value=$product.price span_id="product_price_`$obj_prefix``$obj_id`" class="ty-list-price ty-nowrap"})</span>
                {* Modified by tommy from cs-cart.jp 2017 EOF *}
            {/if}
...
{capture name="list_discount_`$obj_id`"}
    {* Modified by tommy from cs-cart.jp 2017 BOF *}
    {* 外税の場合は割引額を表示しない（税抜価格に対する割引額が表示されると混乱を招くため *}
    {if $show_price_values && $show_list_discount && $details_page && !$jp_disable_you_save}
    {* Modified by tommy from cs-cart.jp 2017 EOF *}
...
{capture name="discount_label_`$obj_prefix``$obj_id`"}
    {if $show_discount_label && ($product.discount_prc || $product.list_discount_prc) && $show_price_values}
        <span class="ty-discount-label cm-reload-{$obj_prefix}{$obj_id}" id="discount_label_update_{$obj_prefix}{$obj_id}">
            {* Modified by tommy from cs-cart.jp 2017 BOF *}
            <span class="ty-discount-label__item" id="line_prc_discount_value_{$obj_prefix}{$obj_id}"><span class="ty-discount-label__value" id="prc_discount_value_label_{$obj_prefix}{$obj_id}">{__("save_discount")} {if $product.discount}{$product.discount_prc}{else}{$product.list_discount_prc}{/if}%{if $smarty.const.CART_LANGUAGE == 'ja'}OFF{/if}</span></span>
            {* Modified by tommy from cs-cart.jp 2017 EOF *}
                        ...
                        {if (
                            ...
                            <div class="ty-control-group product-list-field">
                                {if $show_amount_label}
                                    {* Modified by tommy from cs-cart.jp 2017 BOF *}
                                    <label class="ty-control-group__label">{__("jp_availability")}:</label>
                                    {* Modified by tommy from cs-cart.jp 2017 EOF *}
                                {/if}
            ...
            {else}
                {if (
                    ...
                    <div class="ty-control-group product-list-field">
                        ...
                        {/if}
                        {* Modified by tommy from cs-cart.jp 2017 BOF *}
                        {if $settings.General.allow_negative_amount == "Y" && $product_amount <= 0}
                        <span class="ty-qty-out-of-stock ty-control-group__item" id="out_of_stock_info_{$obj_prefix}{$obj_id}">{$out_of_stock_text}{__("jp_order_allowed")}</span>
                        {else}
