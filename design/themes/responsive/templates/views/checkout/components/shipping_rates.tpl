            ...
            {if $group.shippings && !$group.all_edp_free_shipping && !$group.shipping_no_required}
                ...
                {foreach $all_shippings.$group_key as $shipping_id => $item}
                    ...
                    {* Modified by takahashi from cs-cart.jp 2019 EOF *}
                        ...
                        {if $ua|regex_replace:'/.*iphone.*/i':'iphone' eq 'iphone' || $ua|regex_replace:'/.*ipad.*/i':'ipad' eq 'ipad' || $ua|regex_replace:'/.*ipod.*/i':'ipod' eq 'ipod'}
                            </label>
                        {/if}
                        <div>
                            {if $delivery_timing|@count}
                                {if $delivery_timing.delivery_time_array|@count}
                                    <div class="jp_delivery_option">
                                        ...
                                        <select id="delivery_time_selected_{$group_key}_{$shipping.shipping_id}" name="delivery_time_selected_{$group_key}_{$shipping.shipping_id}" onchange="fn_calculate_total_shipping_cost(); $.ceLiteCheckout('toggleAddress', {if $shipping.is_address_required == "Y"}true{else}false{/if});">
                                ...
                                {if $delivery_timing.delivery_date_array|@count}
                                    <div class="jp_delivery_option">
                                        ...
                                        <select id="delivery_date_selected_{$group_key}_{$shipping.shipping_id}" name="delivery_date_selected_{$group_key}_{$shipping.shipping_id}" onchange="fn_calculate_total_shipping_cost(); $.ceLiteCheckout('toggleAddress', {if $shipping.is_address_required == "Y"}true{else}false{/if});">
