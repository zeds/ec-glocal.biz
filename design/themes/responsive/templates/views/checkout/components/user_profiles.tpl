    ...
    {foreach $user_profiles as $profile}
        <div class="ty-tiles litecheckout__field litecheckout__field--xsmall">
            ...
            >
				...
				<p class="ty-tiles__title">{$profile.profile_name}</p>
				
                {if $smarty.const.CART_LANGUAGE == 'ja'}
                    {if $profile.s_zipcode}
                        <p class="ty-tiles__text">〒{$profile.s_zipcode}</p>
                    {/if}
                    {if $profile.s_city || $profile.s_state_descr}
                        <p class="ty-tiles__text">
                    {if $profile.s_state_descr}{$profile.s_state_descr}{/if}{if $profile.s_city}{$profile.s_city}{/if}</p>
                    {/if}
                    <p class="ty-tiles__text">{$profile.s_address} {$profile.s_address_2}</p>
                {else}
