...
<div class="litecheckout__field cm-field-container {$wrapper_class} {$field_type_class_postfix}"
    ...
    {else}
        ...
        <input class="litecheckout__input{if $field.class} {$field.class}{/if}{$input_meta} {if $field.field_type == "ProfileFieldTypes::POSTAL_CODE"|enum}{if $field.class == "shipping-zip-code"}jp_zipcode_change{/if}{/if}"
         {* Modified by takahashi from cs-cart.jp 2019 EOF *}
               placeholder=" "
               id="{$field_id}"
                {* Modified by takahashi from cs-cart.jp 2020 BOF *}
               type={if $field.field_type == "ProfileFieldTypes::POSTAL_CODE"|enum}"tel"{else}{$type}{/if}
               ...
               title="{$field.description}"
               {* Modified by takahashi from cs-cart.jp 2019 BOF *}
               {if $field.field_type == "ProfileFieldTypes::POSTAL_CODE"|enum}
                   {if $field.class == "shipping-zip-code"}
    ...
    {/if}
    {* Modified by takahashi from cs-cart.jp 2020 BOF *}
    <label class="litecheckout__label {$label_meta} cm-trim" for="{$field_id}">{$field.description}</label>
    {* Modified by takahashi from cs-cart.jp 2019 EOF *}
</div>