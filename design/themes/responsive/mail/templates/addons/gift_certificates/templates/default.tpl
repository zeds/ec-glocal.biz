                                        ...
                                        <div style="padding: 15px 0px 0px 18px; color: #ffffff">
                                            ...
                                            <p style="color: #ffffff; padding: 5px 0px 10px 0px; margin: 0px; border-bottom: 1px dashed #72c0f5;">
                                                {if $smarty.const.CART_LANGUAGE == "ja"}
                                                    {__("gift_cert_to")}: {$gift_cert_data.recipient} {__("dear")}
                                                {else}
                                                ...
                                                {if $gift_cert_data.phone}<bdi>{$gift_cert_data.phone}</bdi><br />{/if}
                                        ...
                                        <div style="margin: 12px 0px 0px 10px;">
                                            ...
                                            <table border="0" cellpadding="10" cellspacing="10">
                                                ...
                                                {foreach from=$gift_cert_data.products item="product"}
                                                    <tr valign="top">
                                                        <td><span style="color: #ffffff; line-height: 20px; font-weight: bold">{$product.amount}</span>&nbsp;</td><td><a href="{"products.view?product_id=`$product.product_id``$suffix`"|fn_url:'C':'http'}" class="product-link" style="color: #ffffff; line-height: 20px;"><bdi>{$product.product}</bdi></a><br />
                                    ...
                                    <td background="{$images_dir}/gift_m.png" style="background-repeat: no-repeat; height: 52px;">
                                        <div style="text-align: right;color: #ffffff; padding: 30px 5px 5px 0px">
                                            {if "ULTIMATE"|fn_allowed_for}
                                                {assign var="suffix" value="?`$company`"}
                                            {/if}
                                            <b>{__("shop_now")}:</b>&nbsp;<a href="{$suffix|fn_url:'C':'http'}" target="_blank" class="action-text-button" style="white-space: nowrap; margin-right: 1px; text-decoration: underline; padding: 2px 5px 2px 0px; color: #ffffff;">{$suffix|fn_url:'C':'http'|puny_decode}</a>
                                        </div>
                                    </td>
                                </tr>
                            </table>
